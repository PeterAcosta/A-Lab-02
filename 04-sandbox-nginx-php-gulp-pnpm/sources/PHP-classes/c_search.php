...
#-------------------------------------------------------------------------------------------------------------
# Class name    : c_search
# Class created : Lunes 27 de Julio de 2009 por Peter Acosta
# Class description : Maneja lo relativo a las busquedas
#-------------------------------------------------------------------------------------------------------------
#1. __construct()
#2. __destruct()
#3. 
#4. guardar_en_central($cadena)
#5. diario_a_central()
#6. central_a_top($cantidad)
#7. cadena_value($cadena)
#8. calculos_diarios()
#9. lee_top($cantidad)
#10 cant_blocks_keyword($keys)
...
class c_search
{
    ...
    #------------------------------------------------------------------------------------------------------------------------------------------------4
    # function   : guardar_en_total($cadena)
    # Created    : Lunes 27 de Julio de 2009 por Peter Acosta
    # Parameters : STRing $cadena
    # Return     : NOne
    # Function   : Guarda las busquedas tabla search_central palabra por palabra, suma +1 cada vez
    #-------------------------------------------------------------------------------------------------------------------------------------------------
    public function guardar_en_central($action, $cadena)
    {
        #echo "<br />Cadena : \"$cadena\" --- from : \"$from\" --- ";
    ...
    #---------------------------------------------------------------------------------------------------------5
    # function   : guardar_diario($cadena)
    # Created    : Lunes 27 de Julio de 2009 por Peter Acosta
    # Parameters : None
    # Return     : NOne
    # Function   : pasa las busquedas de la tabla diaria a la tabla total
    #---------------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------------6
    # function   : central_a_top($cantidad)
    # Created    : Lunes 27 de Julio de 2009 por Peter Acosta
    # Parameters : INT $cantidad , los primeros n elementos rankiados segun prodio
    # Return     : NOne
    # Function   : Guarda las n palabras mas buscadas en la tabla search_top
    #---------------------------------------------------------------------------------------------------------
    public function central_a_top($cantidad)
    {
        #Borro completamente la tabla search_top
        ...
        # Leo los primeros n registros de la tabla search central
    ...
    #---------------------------------------------------------------------------------------------------------7
    # function   : cadena_value($cadena)
    # Created    : Lunes 27 de Julio de 2009 por Peter Acosta
    # Parameters : STRing $cadena
    # Return     : INT con el valor de la cadena
    # Function   : Dice cual es el valor de una cadena segun la cantidad de palabras que posea
    #---------------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------------8
    # Function name : calculos_diarios()
    # Function Date : viernes 19 de diciembre de 2009 por Peter Acosta
    # Parameters : none
    # Return     : none
    # Function	 : actualiza en la tabla "search" diversos campos
    #---------------------------------------------------------------------------------------------------------
    function calculos_diarios()
    {
        ...
        $sql = 'UPDATE search_central SET';
        $sql .= ' days_sum=DATEDIFF(CURRENT_DATE(),search_day)';
        $sql .= ", search_points= ( @aux1 :=  ((search_times*search_value*$valor_del_search)+(link_times*$valor_del_link))*search_coef )  ";
        $sql .= ', search_average= @aux1 /IF(days_sum>1,days_sum,2)';
        $nada = mysqli_query($GLOBALS['link'], $sql) or die("<br />ERROR - c_search->calculos_diarios() - $sql <br />" . mysqli_error($GLOBALS['link']));
        */
    ...
    #---------------------------------------------------------------------------------------------------------9
    # Function name : lee_top($cantidad)
    # Function Date : martes 28 de Julio de 2009 por Peter Acosta
    # Parameters    : INT con la cantidad que se quiere recuperar
    # Return        : ARRAY con los top search
    #---------------------------------------------------------------------------------------------------------
    ...
    /** ** *******************************************************************************************************************************10
     * @example cant_blocks_keyword($keys)
     * @since   Jueves 30 de mayo de 2019
     * @uses calcula la cantidad de bloques que existen que tengan estas keywords
     * @author  Peter Acosta
     * @param string $keys
     * @return int cant_block
     */
